...
def chunk_max(seq, n, max_chunk_sum):
    out = []
    for _ in range(int(np.ceil(len(seq) / max_chunk_sum))):
        out.append([c for c in chunks(seq[:max_chunk_sum], n) if c])
        seq = seq[max_chunk_sum:]
    return out
